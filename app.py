...
def download_results(df, trade_data=None):
...
def macd_optimization_app():
    ...
    if uploaded_file is not None:
        ...
        max_days = st.number_input("Max Trading Days to Hit Target (set 0 for no limit)", min_value=0, max_value=1000, value=10, step=1)
        ...
        if st.session_state.running:
            ...
            if results:
                ...
                st.dataframe(top10)
                download_results(top10, trade_data={
                    (row.FastEMA, row.SlowEMA, row.SignalEMA): trade_details[(row.FastEMA, row.SlowEMA, row.SignalEMA)]
